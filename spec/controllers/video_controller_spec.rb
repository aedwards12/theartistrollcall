...
RSpec.describe VideoController, type: :controller do
  ...
  describe "#tag" do
    it "creates and artist_video" do
      vid = create(:video)
      params = {artist_role: "dancer(s)", dancer_tags: ["AnthonyEdwardsj"], commit: "Add Artist", video_id: vid.id, handle_type: {'0': 'twitter'}, xhr: true}
      VCR.use_cassette 'video_tag_new_cassette' do
        xhr :post, :tag, params
      end
